...
public class kafkaTestJob {
    ...
    public static void main(String[] args) throws Exception {
        ...
        tableEnv.sqlUpdate("CREATE TABLE taxi_trip_data_t3( \n" +
                "vendorid VARCHAR,\n"+
        ...
        tableEnv.sqlUpdate("CREATE TABLE fact_taxi_trip_data_5( \n" +
                "vendorid VARCHAR,\n"+
                "tpep_pickup_datetime VARCHAR,\n"+
                "tpep_dropoff_datetime VARCHAR,\n"+
                "passenger_count SMALLINT,\n"+
                ...
                "'connector.table' = 'fact_taxi_trip_data_1', \n" +
        ...
        tableEnv.sqlUpdate(
                "INSERT INTO fact_taxi_trip_data_5  select * from taxi_trip_data_t3  where trip_distance > 0 and total_amount >0 and payment_type in (1,2,3,4,5,6) and vendorid in (1,2) and ratecodeid  in (1,2,3,4,5,6) and tpep_pickup_datetime<tpep_dropoff_datetime and tip_amount >= 0 and fare_amount >= 0 and extra >= 0 and mta_tax >= 0 and tolls_amount >= 0 and improvement_surcharge >= 0  and total_amount >= 0 and (fare_amount+extra+mta_tax+tip_amount+tolls_amount+improvement_surcharge) = total_amount");
        tableEnv.execute("模拟将合法的历史数据写入到事实表");
